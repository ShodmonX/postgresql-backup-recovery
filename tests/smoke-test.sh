...
run_vagrant() {
  ...
  vagrant ssh "${vm}" -c "${command}" -- -T | tr -d '\r'
}
...
psql_query() {
  ...
  run_vagrant "${vm}" \
    "echo ${encoded} | base64 -d | sudo -u postgres psql -v ON_ERROR_STOP=1 -d ${database} -Atq"
}
...
test_vm() {
  ...
  if ! vagrant ssh "${vm}" -c "hostname" -- -T >/dev/null 2>&1; then
    echo "ERROR: ${vm} is not reachable through Vagrant"
    exit 1
  fi
...
if ! vagrant ssh "${PRIMARY_VM}" -c "pg_isready -q" -- -T; then
  echo "ERROR: PostgreSQL primary is not accepting connections"
  exit 1
fi

PRIMARY_RECOVERY_STATE="$(
  psql_query "${PRIMARY_VM}" "SELECT pg_is_in_recovery();"
)"
...
if ! vagrant ssh "${REPLICA_VM}" -c "pg_isready -q" -- -T; then
  echo "ERROR: PostgreSQL replica is not accepting connections"
  exit 1
fi

REPLICA_RECOVERY_STATE="$(
  psql_query "${REPLICA_VM}" "SELECT pg_is_in_recovery();"
)"
...
STREAMING_REPLICA_COUNT="$(
  psql_query "${PRIMARY_VM}" "${STREAMING_SQL}"
)"
...
REPLICA_SYNC_STATE="$(
  psql_query "${PRIMARY_VM}" "${SYNC_STATE_SQL}"
)"
...
REPLICA_SLOT_STATE="$(
  psql_query "${PRIMARY_VM}" "${REPLICA_SLOT_SQL}"
)"
...
BARMAN_SLOT_STATE="$(
  psql_query "${PRIMARY_VM}" "${BARMAN_SLOT_SQL}"
)"
...
# ------------------------------------------------------------
# 6. Barman + recovery-node baseline
# ------------------------------------------------------------
